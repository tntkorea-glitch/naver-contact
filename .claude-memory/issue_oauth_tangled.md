---
name: OAuth 설정 꼬임 (liketica vs contica)
description: Google Cloud Console에 liketica/contica OAuth 클라이언트가 섞여있어 다음 세션에 정리 필요
type: project
originSessionId: 1a01f8a2-e5a3-45c4-abec-d1b739feaec5
---
Google Cloud Console OAuth 설정에 **liketica**와 **contica** 관련 클라이언트/프로젝트가 섞여있어 꼬인 상태로 발견됨 (2026-04-15).
...
- 이게 어느 프로젝트(liketica? contica?) 소속인지 불명확
...
2. 상단 프로젝트 선택기 눌러서 liketica / contica 등 관련 프로젝트 **전부 목록화** — 어느 프로젝트가 진짜인지 사용자와 같이 확인
3. 사용하지 않는 프로젝트의 OAuth 클라이언트는 정리(삭제) — 혼란 요소 제거
4. 살릴 프로젝트에 `Contica Web` OAuth 클라이언트 **새로 만들거나 기존 것 검증**
   - 승인된 JavaScript 출처: `https://contica-contact.vercel.app`, `http://localhost:3006`
...
6. 구글 로그인 테스트
7. 나중에 `contica.co.kr` 도메인 구매 후 JavaScript 출처에 추가
